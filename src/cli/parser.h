...
//
// Authors: 4500ne
// Github: https://github.ccs.neu.edu/euhlmann/CS4500-A1-part1
// Adapted to fit this project's data structures
...
class SorParser : public Object {
   public:
     ...
     */
    virtual void _appendField(StrSlice slice, size_t field_num, Column** columns) {
        slice.trim(SPACE);

        Column* column = columns[field_num];
        ...
        if (column->get_type() == 'S') {
            ...
            dynamic_cast<StringColumn*>(column)->push_back(slice.toString());
        }
        else if (column->get_type() == 'I') {
            dynamic_cast<IntColumn*>(column)->push_back(slice.toInt());
        }
        else if (column->get_type() == 'F') {
            dynamic_cast<FloatColumn*>(column)->push_back(slice.toFloat());
        }
        else if (column->get_type() == 'B') {
    ...
    virtual void _guessFieldType(StrSlice slice, size_t field_num) {
        ...
        // ** If this breaks for any reason, make sure the characters
        // line up with the enums in 4500ne's repo
     ...
     */
    virtual size_t _scanLine(const char* line, ParserMode mode, Column** columns) {
        size_t num_fields = 0;
    ...
    virtual void guessSchema() {
        ...
        _reader->reset();
        _columns = new Column*[max_columns]; 
        ...
        for (size_t i = 0; i < _num_columns; i++) {
            ...
            if (_typeGuesses[i] == "S") {
                _columns[i] = new StringColumn();
            }
            else if (_typeGuesses[i] == "B") {
                    _columns[i] = new BoolColumn();
            }
            else if (_typeGuesses[i] == "I") {
                    _columns[i] = new IntColumn();
            }
            else if (_typeGuesses[i] == "F") {
                    _columns[i] = new FloatColumn();
            }
            else {
                printf("Bad type on parsing");
                exit(1);
            }
        }
    }
    ...
    virtual void parseFile() {
        ...
        while (true) {
            ...
            for (size_t i = scanned_fields; i < _num_columns; i++) {
                _columns[i]->appendMissing();
            }
            delete[] line;
        }
    }

    // Create dataframe from the parsed file; df pointer is external
    virtual DataFrame* getDataFrame() {
        Schema* s = new Schema();
        DataFrame* df = new DataFrame(*s);
        for (int i = 0; i < _num_columns; i++) {
            df->add_column(_columns[i], new String("daf"));
        }
        return df;
    }
